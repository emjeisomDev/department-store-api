...
COMMENT
ON TABLE tb_audit_trail IS 'Tabela central de auditoria para todas as entidades. Implementa soft delete e rastreamento de alteracoes.';
COMMENT
ON COLUMN tb_audit_trail.id IS 'Chave primaria - identificador unico do registro de auditoria';
COMMENT
ON COLUMN tb_audit_trail.table_name IS 'Nome da tabela auditada';
COMMENT
ON COLUMN tb_audit_trail.record_id IS 'ID do registro na tabela referenciada';
COMMENT
ON COLUMN tb_audit_trail.created_at IS 'Data/hora de criacao do registro';
COMMENT
ON COLUMN tb_audit_trail.updated_at IS 'Data/hora da ultima atualizacao';
COMMENT
ON COLUMN tb_audit_trail.deleted_at IS 'Data/hora do soft delete (NULL = registro ativo)';
COMMENT
ON COLUMN tb_audit_trail.created_by IS 'Usuario/sistema que criou o registro';
COMMENT
ON COLUMN tb_audit_trail.updated_by IS 'Usuario/sistema que atualizou o registro';
COMMENT
ON COLUMN tb_audit_trail.deleted_by IS 'Usuario/sistema que removeu o registro';
...
COMMENT
ON TABLE tb_person IS 'Entidade base para todas as pessoas do sistema (fisicas e juridicas)';
COMMENT
ON COLUMN tb_person.id IS 'Chave primaria - identificador unico da pessoa';
COMMENT
ON COLUMN tb_person.name IS 'Nome da pessoa ou nome fantasia se for pessoa juridica';
COMMENT
ON COLUMN tb_person.person_type IS 'F = Pessoa Fisica, J = Pessoa Juridica';
COMMENT
ON COLUMN tb_person.registration_date IS 'Data do registro da pessoa no sistema';
COMMENT
ON COLUMN tb_person.audit_id IS 'Referencia para a tabela de auditoria';
...
COMMENT
ON TABLE tb_natural_person IS 'Especializacao de tb_person para pessoas fisicas';
COMMENT
ON COLUMN tb_natural_person.id IS 'Chave primaria';
COMMENT
ON COLUMN tb_natural_person.person_id IS 'Referencia para tb_person (relacionamento 1:1)';
COMMENT
ON COLUMN tb_natural_person.tax_id IS 'CPF - 11 digitos numericos';
COMMENT
ON COLUMN tb_natural_person.birth_date IS 'Data de nascimento';
COMMENT
ON COLUMN tb_natural_person.mothers_name IS 'Nome da mae da pessoa';
COMMENT
ON COLUMN tb_natural_person.gender IS 'F=Feminino, M=Masculino, O=Outro, N=Nao informar';
...
COMMENT
ON TABLE tb_legal_person IS 'Especializacao de tb_person para pessoas juridicas';
COMMENT
ON COLUMN tb_legal_person.id IS 'Chave primaria';
COMMENT
ON COLUMN tb_legal_person.person_id IS 'Referencia para tb_person (relacionamento 1:1)';
COMMENT
ON COLUMN tb_legal_person.tax_id IS 'CNPJ - 14 digitos numericos';
COMMENT
ON COLUMN tb_legal_person.corporate_name IS 'Razao social da pessoa juridica';
COMMENT
ON COLUMN tb_legal_person.share_capital IS 'Capital social da empresa';
COMMENT
ON COLUMN tb_legal_person.employees_quant IS 'Numero de funcionarios da empresa';
...
COMMENT
ON TABLE tb_legal_person_responsibles IS 'Tabela associativa entre Pessoa Juridica e seus responsaveis (Pessoas Fisicas)';
COMMENT
ON COLUMN tb_legal_person_responsibles.id IS 'Chave primaria';
COMMENT
ON COLUMN tb_legal_person_responsibles.legal_person_id IS 'Referencia para tb_legal_person (PJ)';
COMMENT
ON COLUMN tb_legal_person_responsibles.natural_person_id IS 'Referencia para tb_natural_person (responsavel PF)';
COMMENT
ON COLUMN tb_legal_person_responsibles.responsibility_type IS 'Tipo de responsabilidade (ex: LEGAL_REPRESENTATIVE, ADMINISTRATOR)';
COMMENT
ON COLUMN tb_legal_person_responsibles.start_date IS 'Data inicio da responsabilidade';
COMMENT
ON COLUMN tb_legal_person_responsibles.end_date IS 'Data fim da responsabilidade (NULL = ativo)';
...
COMMENT
ON TABLE tb_employee IS 'Role de funcionario - especializacao comportamental de tb_person';
COMMENT
ON COLUMN tb_employee.id IS 'Chave primaria';
COMMENT
ON COLUMN tb_employee.person_id IS 'Referencia para tb_person (relacionamento 1:1)';
COMMENT
ON COLUMN tb_employee.registration_number IS 'Numero da matricula do funcionario';
COMMENT
ON COLUMN tb_employee.hire_date IS 'Data de admissao do funcionario';
COMMENT
ON COLUMN tb_employee.termination_date IS 'Data de demissao (NULL = ativo)';
COMMENT
ON COLUMN tb_employee.termination_reason IS 'Motivo da demissao';
COMMENT
ON COLUMN tb_employee.employee_role IS 'Funcao do empregado (ex: VENDEDOR, RH, ENGENHEIRO)';
COMMENT
ON COLUMN tb_employee.status IS 'Status do funcionario (ACTIVE, INACTIVE)';
...
COMMENT
ON TABLE tb_client IS 'Role de cliente - especializacao comportamental de tb_person';
COMMENT
ON COLUMN tb_client.id IS 'Chave primaria';
COMMENT
ON COLUMN tb_client.person_id IS 'Referencia para tb_person (relacionamento 1:1)';
COMMENT
ON COLUMN tb_client.client_code IS 'Codigo unico do cliente no sistema';
COMMENT
ON COLUMN tb_client.client_rank IS 'Classificacao do cliente (ex: STANDARD, PREMIUM, VIP)';
COMMENT
ON COLUMN tb_client.status IS 'Status do cliente (ACTIVE, INACTIVE)';
...
CREATE
OR REPLACE FUNCTION fn_update_timestamp()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at
= CURRENT_TIMESTAMP;
...
CREATE
OR REPLACE FUNCTION fn_audit_insert()
RETURNS TRIGGER AS $$
DECLARE
v_audit_id BIGINT;
    v_table_name
...
VALUES (v_table_name, NEW.id, CURRENT_TIMESTAMP,
        COALESCE(current_setting('app.current_user', true), 'system')) RETURNING id
INTO v_audit_id;
...
CREATE
OR REPLACE FUNCTION fn_audit_update()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE
OR REPLACE FUNCTION fn_audit_soft_delete()
RETURNS TRIGGER AS $$
BEGIN
UPDATE tb_audit_trail
...
CREATE
OR REPLACE FUNCTION fn_validate_person_type()
RETURNS TRIGGER AS $$
DECLARE
v_count_f INTEGER;
    v_count_j
...
CREATE
OR REPLACE FUNCTION fn_prevent_duplicate_person_type()
RETURNS TRIGGER AS $$
DECLARE
...
IF
TG_TABLE_NAME = 'tb_natural_person' AND v_person_type = 'J' THEN
...
END IF;
    ...
    IF
TG_TABLE_NAME = 'tb_legal_person' AND v_person_type = 'F' THEN
...
CREATE
OR REPLACE FUNCTION fn_validate_legal_person_responsible()
RETURNS TRIGGER AS $$
DECLARE
v_person_type CHAR(1);
BEGIN
...
CREATE
OR REPLACE FUNCTION fn_validate_employee_natural_person()
RETURNS TRIGGER AS $$
DECLARE
v_person_type CHAR(1);
BEGIN
...
CREATE TRIGGER trg_validate_responsible
    BEFORE INSERT OR
UPDATE ON tb_legal_person_responsibles
    FOR EACH ROW
...
CREATE
OR REPLACE VIEW vw_person_complete AS
...
FROM tb_person p
         ...
         LEFT JOIN tb_audit_trail at
ON p.audit_id = at.id;

COMMENT
ON VIEW vw_person_complete IS 'Visao completa unificada de todas as pessoas com dados de PF, PJ, roles e auditoria';
...
COMMENT
ON VIEW vw_active_employees IS 'Lista de funcionarios ativos no sistema (sem demissao e nao excluidos)';
...
COMMENT
ON VIEW vw_active_clients IS 'Lista de clientes ativos no sistema (nao excluidos)';

-- View: Responsaveis por Pessoa Juridica
CREATE
OR REPLACE VIEW vw_legal_person_responsibles AS
SELECT lpr.id,
...
COMMENT
ON VIEW vw_legal_person_responsibles IS 'Lista de responsaveis vinculados a cada Pessoa Juridica';

-- View: Status de auditoria
CREATE
OR REPLACE VIEW vw_audit_status AS
SELECT at.id                                                                          AS audit_id,
       at.table_name,
       at.record_id,
       at.created_at,
       at.updated_at,
...
COMMENT
ON VIEW vw_audit_status IS 'Visao geral do status de auditoria de todos os registros';
...
CREATE
OR REPLACE PROCEDURE sp_soft_delete_person(p_person_id BIGINT, p_deleted_by VARCHAR(100) DEFAULT 'system')
LANGUAGE plpgsql
AS $$
BEGIN
UPDATE tb_audit_trail
...
COMMENT
ON PROCEDURE sp_soft_delete_person IS 'Executa soft delete de uma pessoa pelo ID';

-- Procedure: Restaurar pessoa excluida (soft delete)
CREATE
OR REPLACE PROCEDURE sp_restore_person(p_person_id BIGINT)
LANGUAGE plpgsql
AS $$
BEGIN
UPDATE tb_audit_trail
...
COMMENT
ON PROCEDURE sp_restore_person IS 'Restaura uma pessoa previamente soft-deleted';